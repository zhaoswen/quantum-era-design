...
import {info} from "../tools/message.ts";
...
export const loadProjectDependencies = async (dependencies: Dependency[]) => {
    ...
    for (let index in dependencies) {
        ...
        // 拼接插件处理器配置目录
        let handler_path =
            simx_root_dir + sep() + "extension" + sep() + dependency_path.name + sep() + "handler";
        // 判断handler_path是否存在
        let isHandlerPathExist: boolean = await invoke("dir_exists", {
            path: handler_path,
        });
        if (!isHandlerPathExist) {
            info("插件处理器配置不存在，正在尝试下载");
            // 配置不存在，尝试下载
...
export const RequireExtensionProfile = async (extension_path: string) => {
    ...
    let simx_extension_path =
        runtime.engineCorePath + sep() + "extension" + sep() + extension_path;
    ...
    // 1. 创建插件描述文件
    invoke("write_file", {
        path: simx_extension_path + sep() + "index.json",
        content: JSON.stringify(response.data.info),
    });

    // 2. 创建基础文件夹
    invoke("dir_create", {path: simx_extension_path + sep() + "handler"});
    invoke("dir_create", {path: simx_extension_path + sep() + "service"});
    ...
    if (handlers) {
        for (let i = 0; i < handlers.length; i++) {
            ...
            invoke("write_file", {
                path:
                    simx_extension_path + sep() +
                    "handler" + sep() +
                    group.id.split(".").pop() + sep() +
                    group.id.split(".").pop() +
                    ".json",
